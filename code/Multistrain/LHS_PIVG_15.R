...
# LHS scan - Infected model - 15 minute latent period

L1 = 1/4 # latent period (hours)
eta1=1/L1  #lysis rate of infected cells (per hour)
...
for(aa in 1:samples){ #for each sample
	...
	if(Nstar>0 & Vstar>0 & Istar>0){
		...
		GG1_b_vdil = ANALYSIS(GNIV,model_params,model_state,c(1,2),c(3),Dil_levels,0,PROJECTINFO) 
...
dev.copy2eps(file=paste("../../figures/densityPIVG_15m_24h",".eps",sep=""))
dev.copy2pdf(file=paste("../../figures/densityPIVG_15m_24h",".pdf",sep=""))
dev.copy(jpeg,file=paste("../../figures/densityPIVG_15m_24h",".jpg",sep=""))
dev.off()
dev.off()
...
dev.copy2eps(file=paste("../../figures/densityPIVG_15m_2h",".eps",sep=""))
dev.copy2pdf(file=paste("../../figures/densityPIVG_15m_2h",".pdf",sep=""))
dev.copy(jpeg,file=paste("../../figures/densityPIVG_15m_2h",".jpg",sep=""))
dev.off()
dev.off()
...
dev.copy2eps(file=paste("../../figures/densityPIVG_15m_both",".eps",sep=""))
dev.copy2pdf(file=paste("../../figures/densityPIVG_15m_both",".pdf",sep=""))
dev.copy(jpeg,file=paste("../../figures/densityPIVG_15m_both",".jpg",sep=""))
dev.off()
dev.off()
